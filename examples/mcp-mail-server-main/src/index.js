...
const imap_client_js_1 = require("./imap-client.js");
const smtp_client_js_1 = require("./smtp-client.js");
...
class MailMCPServer {
    ...
    isSmtpInitializing = false;
    // undefined = 未查找过，null = 查找过但找不到，string = 已找到的真实路径
    sentMailboxName = undefined;
    ...
    filterMessagesByDateRange(messages, startDate, endDate) {
        ...
        const filtered = messages.filter(msg => {
            if (!msg.date)
                return true; // 如果没有日期信息，保留邮件
            const msgDate = new Date(msg.date);
            if (isNaN(msgDate.getTime()))
                return true; // 日期解析失败，保留邮件
    ...
    // 在多个邮箱中搜索的辅助方法
    async searchInMultipleMailboxes(criteria, searchType, searchValue, startDate = '', endDate = '', inboxOnly = false, limit) {
        ...
        // 按时间降序排序（最新的邮件在前）
        searchResults.messages.sort((a, b) => {
            ...
            try {
                switch (name) {
                    ...
                    case 'get_unseen_messages':
                        return await this.handleGetUnseenMessages(args?.limit);
                    case 'get_recent_messages':
                        return await this.handleGetRecentMessages(args?.limit);
    ...
    async ensureIMAPConnection() {
        ...
        try {
            const config = config_js_1.EMAIL_CONFIG.IMAP;
            console.error(`[IMAP] Auto-connecting to ${config.host}:${config.port}`);
            this.imapClient = new imap_client_js_1.IMAPClient(config);
    ...
    async ensureSMTPConnection() {
        ...
        try {
            const config = config_js_1.EMAIL_CONFIG.SMTP;
            console.error(`[SMTP] Auto-connecting to ${config.host}:${config.port}`);
            this.smtpClient = new smtp_client_js_1.SMTPClient(config);
    ...
    async ensureRequiredConnections(requireIMAP = false, requireSMTP = false) {
        if (requireIMAP) {
    ...
    async findSentMailbox() {
        ...
        try {
            ...
            const findByAttrib = (nodes, prefix = '') => {
    ...
    async handleListMailboxes() {
        await this.ensureRequiredConnections(true, false);
        try {
            const boxes = await this.imapClient.getBoxes();
            // 处理循环引用，创建一个简化的邮箱列表
            const processMailbox = (box, path = '', visited = new Set()) => {
                if (visited.has(box)) {
    ...
    async handleSearchBySender(args) {
        await this.ensureRequiredConnections(true, false);
    ...
    async handleSearchBySubject(args) {
        await this.ensureRequiredConnections(true, false);
    ...
    async handleSearchByRecipient(args) {
        await this.ensureRequiredConnections(true, false);
    ...
    async handleSearchSinceDate(args) {
        await this.ensureRequiredConnections(true, false);
    ...
    async handleSearchUnreadFromSender(args) {
        await this.ensureRequiredConnections(true, false);
        const sender = args.sender;
        const startDate = args.startDate || '';
        const endDate = args.endDate || '';
    ...
    async handleSearchUnrepliedFromSender(args) {
        await this.ensureRequiredConnections(true, false);
        const sender = args.sender;
        const startDate = args.startDate || '';
        const endDate = args.endDate || '';
        ...
        try {
            ...
            // 1. 获取来自发件人的邮件（收到的邮件）
            const fromSenderResult = await this.searchInMultipleMailboxes([['FROM', sender]], 'From Sender', sender, startDate, endDate, args.inboxOnly || false, limit);
            // 2. 获取发给该发件人的邮件（已发送的邮件）
            const toSenderResult = await this.searchInMultipleMailboxes([['TO', sender]], 'To Sender', sender, startDate, endDate, args.inboxOnly || false, limit);
            ...
            // 3. 使用改进的算法检测未回复邮件
            const unrepliedMessages = this.detectUnrepliedMessagesAdvanced(fromSenderResult.messages, toSenderResult.messages, sender, limit // 完全按用户设置，不设上限
            );
    ...
    async handleSearchWithKeyword(args) {
        await this.ensureRequiredConnections(true, false);
    ...
    async handleSearchAllMessages(args) {
        await this.ensureRequiredConnections(true, false);
        const startDate = args.startDate || '';
        const endDate = args.endDate || '';
    ...
    async handleGetAttachments(args) {
        await this.ensureRequiredConnections(true, false);
        const uid = args.uid;
        if (typeof uid !== 'number') {
            throw new Error('uid must be a number');
    ...
    async handleSaveAttachment(args) {
        await this.ensureRequiredConnections(true, false);
        const uid = args.uid;
        ...
        if (!path_1.default.isAbsolute(savePath)) {
        ...
        try {
            ...
            for (const att of attachmentsToSave) {
                // 生成安全的文件名（避免路径遍历）
                const safeFilename = path_1.default.basename(att.filename);
                let targetPath = path_1.default.join(savePath, safeFilename);
                // 文件名冲突自动追加序号
                let counter = 1;
                const ext = path_1.default.extname(safeFilename);
                const nameWithoutExt = path_1.default.basename(safeFilename, ext);
                while (true) {
                    try {
                        await (0, promises_1.access)(targetPath);
                        // 文件已存在，追加序号
                        targetPath = path_1.default.join(savePath, `${nameWithoutExt}_${counter}${ext}`);
                        counter++;
                    }
                    catch {
                        // 文件不存在，可以使用
                ...
                await (0, promises_1.writeFile)(targetPath, att.content);
    ...
    async handleGetUnseenMessages(limit) {
        await this.ensureRequiredConnections(true, false);
        try {
    ...
    async handleGetRecentMessages(limit) {
        await this.ensureRequiredConnections(true, false);
        try {
            const fetchLimit = limit && limit > 0 ? limit : 50;
        ...
        const status = {
            ...
            connections: {
                imap: {
                    connected: false,
                    currentBox: null,
                    serverInfo: `${config_js_1.EMAIL_CONFIG.IMAP.host}:${config_js_1.EMAIL_CONFIG.IMAP.port}`,
                    username: config_js_1.EMAIL_CONFIG.IMAP.username,
                    tls: config_js_1.EMAIL_CONFIG.IMAP.tls,
                    status: 'Not connected'
                },
                smtp: {
                    connected: false,
                    serverInfo: `${config_js_1.EMAIL_CONFIG.SMTP.host}:${config_js_1.EMAIL_CONFIG.SMTP.port}`,
                    username: config_js_1.EMAIL_CONFIG.SMTP.username,
                    secure: config_js_1.EMAIL_CONFIG.SMTP.secure,
                    status: 'Not connected'
                }
            }
        };
    ...
    async handleSendEmail(args) {
        ...
        const emailOptions = {
            to: args.to.split(',').map((email) => email.trim()),
            subject: args.subject,
            text: args.text,
            html: args.html,
            cc: args.cc ? args.cc.split(',').map((email) => email.trim()) : undefined,
            bcc: args.bcc ? args.bcc.split(',').map((email) => email.trim()) : undefined,
        };
    ...
    async handleReplyToEmail(args) {
        ...
        try {
            ...
            if (replyToAll) {
                ...
                const filteredRecipients = allRecipients.filter(email => email !== config_js_1.EMAIL_CONFIG.IMAP.username && email !== originalFrom);
    ...
    // 辅助方法：从地址字段提取邮箱地址
    extractEmailFromAddress(addressField) {
        if (!addressField)
            return null;
    ...
    // 辅助方法：从地址字段提取多个邮箱地址
    extractEmailsFromAddressField(addressField) {
        if (!addressField)
            return [];
     ...
     */
    detectUnrepliedMessagesAdvanced(fromSenderMessages, toSenderMessages, senderEmail, maxUnreplied) {
        const unrepliedMessages = [];
        // 按时间排序邮件
        const sortedFromSender = [...fromSenderMessages].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        const sortedToSender = [...toSenderMessages].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
     ...
     */
    isMessageReplied(originalMessage, sentMessages, senderEmail) {
     ...
     */
    isLikelyReplyByContent(originalMessage, potentialReply) {
        const originalSubject = this.cleanReplySubject(originalMessage.subject || '').toLowerCase();
    ...
    // 辅助方法：构建文本格式的引用内容
    buildQuotedText(originalText, date, from) {
    ...
    // 辅助方法：构建HTML格式的引用内容
    buildQuotedHtml(originalContent, date, from) {
    ...
    // 通用方法：在多个邮箱中查找邮件
    async findMessageInMultipleMailboxes(uid) {
    ...
    // 通用方法：确保邮箱已打开
    async ensureMailboxOpen(defaultMailbox = 'INBOX') {
    ...
    async handleConnectAll() {
        ...
        try {
            if (this.imapClient && this.imapClient.isConnected()) {
                ...
                const configUser = config_js_1.EMAIL_CONFIG.IMAP.username;
        ...
        try {
            if (this.smtpClient && this.smtpClient.isConnected()) {
                ...
                const configUser = config_js_1.EMAIL_CONFIG.SMTP.username;
                if (currentUser !== configUser) {
    ...
    validateConfig() {
        try {
            ...
            console.error(`IMAP: ${config_js_1.EMAIL_CONFIG.IMAP.host}:${config_js_1.EMAIL_CONFIG.IMAP.port} (TLS: ${config_js_1.EMAIL_CONFIG.IMAP.tls})`);
            console.error(`SMTP: ${config_js_1.EMAIL_CONFIG.SMTP.host}:${config_js_1.EMAIL_CONFIG.SMTP.port} (Secure: ${config_js_1.EMAIL_CONFIG.SMTP.secure})`);
            console.error(`User: ${config_js_1.EMAIL_CONFIG.IMAP.username}`);
    ...
    buildRawEmailMessage(emailOptions, messageId) {
        const now = new Date();
        const dateString = now.toUTCString();
        // 生成 Message-ID
        const msgId = messageId || `<${Date.now()}.${Math.random().toString(36)}@${config_js_1.EMAIL_CONFIG.IMAP.host}>`;
        let rawMessage = '';
        rawMessage += `Message-ID: ${msgId}\r\n`;
        rawMessage += `Date: ${dateString}\r\n`;
        rawMessage += `From: ${config_js_1.EMAIL_CONFIG.IMAP.username}\r\n`;
    ...
    // 保存已发送邮件到发件箱
    async saveSentMessage(emailOptions, messageId) {
        try {
            await this.ensureRequiredConnections(true, false);
